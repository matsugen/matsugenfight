...
他に、左上に信号(現在は常にNONE)とキー入力を数値化したものを表示し、キーボードの"Q"キーでパンチモーション、方向キーで左右にのみ移動可能。また、試験的に赤い半透明の矩形を表示している。デバッグ用
...
2013/1/14
	-"chara"フォルダを追加。以降このフォルダにキャラクターのクラスを入れる。
	-*_CollisionクラスをHit,Push,Attackとクラス名を変更。これに伴い、lua側で扱うときはcollisionテーブルの領域内のクラスとして扱っている(使用時はcollision.Push()などとしてインスタンス化する)
	-座標(200,0)に動かない棒人間を表示。chara/chara.luaにある"Chara"クラスを用いて判定とスプライトを表示している。表示は	現状Attackのみで、バイナリファイル"test"を読み込むことで座標などを取得している。
	今後の課題
		-バイナリファイルを直接いじるのは面倒なので、GUIツールを作成する必要がある。